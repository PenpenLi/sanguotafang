...
//    NotificationManager is used for handling messages between GameObjects.
...
//    To use NotificationManager, either create and manage a unique instance of it somewhere, or use the static NotificationManager.

// We need a static method for objects to be able to obtain the default notification center.
// This default center is what all objects will use for most notifications.  We can of course create our own separate instances of NotificationManager, but this is the static one used by all.
public class NotificationManager
{
	private static NotificationManager defaultCenter;
	public static NotificationManager getInstance () {
		// If the defaultCenter doesn't already exist, we need to create it
		if (defaultCenter == null) {
			// Because the NotificationManager is a Observer, we have to create a GameObject to attach it to.
			//GameObject notificationObject = new GameObject("Default Notification Center");
			// Add the NotificationManager Observer, and set it as the defaultCenter
			defaultCenter = new NotificationManager();//notificationObject.AddObserver<NotificationManager>();
	...
	public void AddObserver (Observer observer, int name) { AddObserver(observer, name, null); }
	public void AddObserver (Observer observer, int name, object sender) {
		// If the name isn't good, then throw an error and return.
		if (name <= 0) { Debug.Log("Null name specified for notification in AddObserver."); return; }
	...
	public void RemoveObserver (Observer observer, int name) {
	...
	// A notification can either be posted with a notification object or by just sending the individual Observers.
	public void PostNotification (Observer aSender, int aName) { PostNotification(aSender, aName, null); }
	public void PostNotification (Observer aSender, int aName, object aData) { PostNotification(new Notification(aSender, aName, aData)); }
	public void PostNotification (Notification aNotification) {
		...
		if (aNotification.name <= 0) { Debug.Log("Null name sent to PostNotification."); return; }
		...
		foreach (Observer observer in notifyList) {
			...
			if (!observer) { observersToRemove.Add(observer);
				...
				//observer.LocalMessage(aNotification);
				observer.notificationQueue.Add(aNotification);
			}
		}
...
public class Notification {
	...
	public Notification (Observer aSender, int aName) { sender = aSender; name = aName; data = null; }
	public Notification (Observer aSender, int aName, object aData) { sender = aSender; name = aName; data = aData; }
}